...
G20_summary %>% dplyr::filter(`Time period`=="2023") %>% ggplot(., aes(area = `Contr_to_tot`,fill=`Group Name`, 
  ...
  ggtitle("Contribution of individual countries' GHG emissions to G20 emissions")
...
EA_summary %>% dplyr::filter(`Time period`=="2023")   %>% ggplot(., aes(area = `Contr_to_tot`,fill=`Group Name`,
  ...
  ggtitle("Contribution of individual countries' GHG emissions to Euro Area emissions")
...
data_chart2_g20  %>%
  mutate(Sector = fct_reorder(Sector, `contribution`, .fun = sum, .desc = FALSE)) %>% ggplot(., aes(x=`Time period`, y=`contribution`, fill=Sector)) + 
...
colScale2=scale_fill_manual(values = c("#5C5C5C","#00B1EA","#007816","#65b800","#ff4b00","#8139C6","#FFB400","#003899"))
